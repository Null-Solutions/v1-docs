...
Let `R` be Receiver assets and `P` be Payer assets. Payer's **instantaneous leverage** is:

```text
L = R / P
```

`L` is the leverage Payer has at that market state. It is not selected by the trader and it is not fixed when you enter. It changes whenever Receiver assets, Payer assets, funding, fees, or settled price PnL change either pool.

The whole Payer pool takes the opposite side of Receiver notional `R`. A Payer holder whose current position value is `p` therefore has:

```text
Payer notional = p * L = p * R / P
```

If coverage falls, `P` becomes smaller relative to `R` and Payer leverage rises. The same Receiver-side transfer then has a larger percentage effect on Payer share value.
...
At that moment, Payer leverage is `L = 1,000 / 200 = 5×`. Receiver gains 2%. Payer pays the same 20-unit transfer, which reduces its 200-unit pool by 10%:

```text
Payer return ≈ -price move * L
             ≈ -2% * 5
             ≈ -10%
```
...
Each Payer holder receives or pays its pro-rata share of that transfer. A holder with current position value `p` has instantaneous notional `pL`, so its annualized funding transfer is:

```text
holder funding transfer = p * L * f
```

Positive `f` means Receiver pays Payer. Negative `f` means Payer pays Receiver. Leverage changes the holder's notional, not the funding rate `f`. Funding is variable, can reverse, and is not guaranteed yield.
...
<Steps>
  ...
  <Step title="Choose Receiver or Payer">
    Receiver takes the tracked return. Payer takes the opposite return at the current instantaneous leverage `L = R / P`.
  </Step>
